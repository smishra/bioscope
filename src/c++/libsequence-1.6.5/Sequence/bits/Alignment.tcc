  ...
  namespace Alignment
  {
    ...
    */
	...
	return Sequence::SEQMAXUNSIGNED;
      ...
      for (unsigned j = 0; j < data[0].second.length (); ++j)
    ...
    */
    {
       ...
       BOOST_STATIC_ASSERT((boost::is_base_and_derived<std::pair<std::string,std::string>,T>::value
			    || boost::is_same<std::pair<std::string,std::string>,T>::value));
      unsigned i, j,datasize=data.size();
      unsigned length = data[0].second.length ();
    ...
    */
    {
      BOOST_STATIC_ASSERT((boost::is_base_and_derived<std::pair<std::string,std::string>,T>::value
			   || boost::is_same<std::pair<std::string,std::string>,T>::value));
      unsigned i, j;
      unsigned length = data[0].second.length ();	//how much we have to iterate over
      std::vector < std::string > trimmed_sequences;
      unsigned leftmost, rightmost, numUngapped;
      unsigned offset;
    ...
    */
    {
      BOOST_STATIC_ASSERT((boost::is_base_and_derived<std::pair<std::string,std::string>,T>::value
			   || boost::is_same<std::pair<std::string,std::string>,T>::value));
      const unsigned nsam = data.size()-1;
      unsigned s = data[0].second.length()-1,max=s+1;
    ...
    */
    {
      BOOST_STATIC_ASSERT((boost::is_base_and_derived<std::pair<std::string,std::string>,T>::value
			   || boost::is_same<std::pair<std::string,std::string>,T>::value));
      unsigned i, j, numseqs = data.size (), numIntervals =  sites.size ();
      unsigned start, stop;
    ...
    */
    {
      ...
      unsigned i, j, start, stop, numseqs = data.size (), numIntervals = sites.size (), lastval;
