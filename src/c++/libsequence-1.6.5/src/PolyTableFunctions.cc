...
namespace Sequence
{
  ...
  */
  ...
  /*!
    ...
    \throw SeqException if unable to assign transformed data to table (via PolyTable::assign)
  */
  {
  ...
  void RemoveInvariantColumns(PolyTable * table,
			      ...
			      const unsigned & outgroup) throw (SeqException)
  /*!
	...
	for(unsigned ind=0;ind<table->size();++ind)
	  {
	    if ( !skipOutgroup || (skipOutgroup && ind != outgroup) )
	      {
		char uch =  std::toupper((*table)[ind][site]);
		if ( uch != 'N' )
		  sc.insert(uch);
	      }
	  }
