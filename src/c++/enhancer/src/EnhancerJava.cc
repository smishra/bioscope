...
void GetNearByGenes (long Start, long End, string &, list <Annotation *> &Alist, Exon &, EnhancerParam &, string&, string &, string &, bool &, string &) ;
...
void Check (char *filename, char *chromosome, char *session, bool cache) ;
 ...
 void PerformAlone (int argc, char *argv[])
 {
   ...
   EnhancerParam Param ;
    ...
    Check ((char *) Param.GetDirectoryName(), (char *) Param.GetChName(), (char *)Param.GetSessionName(), Param.GetRecalculate()) ;
 ...
 void Check (char *filename, char *chromosome, char *session, bool cache)
 {
  ...
  ReadAnnotationList (GeneFileName, AnnotationList, ExonList)  ;
   ...
   ResultsList.OutputResults (Param, fseq, AnnotationList, ExonList) ;
...
void GetNearByGenes (long Start, long End, string &GeneDetails, vector <Annotation *> &Alist, Exon &ExonList, EnhancerParam &Param, string &done, string &xmlstr, bool &Found, string & imagexml) 
{
     ...
     while (beginpos != Alist.end())
	{
	  ...
	  if (Add)
	      ...
	      else
		{
	      ...
	      sprintf (buffer, "%s<td>%ld</td>", GeneEnd.c_str(), abs ((*beginpos)->GetStartPosition() - (*beginpos)->GetEndPosition())) ;
	      GeneEnd =  buffer ;
	      sprintf (buffer, "%ld\n", (*beginpos)->GetEndPosition()) ;
	      xmlstr +=  buffer ;
	      xmlstr += "</length>\n<strand>\n" ;

	      BuildImageMapStr ((*beginpos)->GetStartPosition(), (*beginpos)->GetEndPosition(),	fixedleft, fixedright, GENE, (*beginpos)->GetForwardStrand(), description, imagexml) ;

	      if ((*beginpos)->GetForwardStrand())
		{
		  sprintf (buffer, "%s<td><center> <img src=\"%sforward.gif\"></center></td>", GeneOrientation.c_str(), WebAddress) ;
		  GeneOrientation = buffer ;
		  xmlstr += "forward\n" ;
		}
	      else
		{
		  sprintf (buffer, "%s<td><center> <img src=\"%sreverse.gif\"></center></td>", GeneOrientation.c_str(), WebAddress) ;
		  GeneOrientation = buffer ;
		  xmlstr += "reverse\n" ;
		}
	      xmlstr += "</strand>\n" ;
  ...
  if (whichway == 'r' || whichway == 'b') 
	...
	
	{
	  ...
	  BuildImageMapStr ((*beginpos)->GetStartPosition(), (*beginpos)->GetEndPosition(), fixedleft, fixedright, GENE, (*beginpos)->GetForwardStrand(), description, imagexml) ;
       ...
       if ((*beginpos)->GetForwardStrand())
	 {
	   GeneOrientation += "<td><center> <img src=\"" ;
	   GeneOrientation += WebAddress ;
	   GeneOrientation += "forward.gif\"></center></td>" ;
	   xmlstr += "forward\n" ;
	 }
       else
       {
	 GeneOrientation += "<td><center> <img src=\"" ;
	 GeneOrientation +=  WebAddress ;
	 GeneOrientation +=  "reverse.gif\"></center></td>" ;
	 xmlstr += "reverse\n" ;
       }
       xmlstr += "</strand>\n" ;
       NotesInfo = (char *) (*beginpos)->GetNotes((char *) "Blast") ;
